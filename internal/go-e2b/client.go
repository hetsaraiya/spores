...
func (c *Client) NewSandbox(ctx context.Context, cfgs ...SandboxConfig) (*Sandbox, error) {
	...
	log.Printf("e2b create sandbox request: url=%s body=%s", c.apiBaseURL+"/sandboxes", string(body))
	...
	if resp.StatusCode != http.StatusCreated {
		...
		log.Printf("e2b create sandbox response: status=%d body=%s", resp.StatusCode, string(respBody))
	...
	if err != nil {
		return nil, fmt.Errorf("e2b: read create response: %w", err)
	}
	log.Printf("e2b create sandbox response: status=%d body=%s", resp.StatusCode, string(respBody))
	...
	log.Printf("e2b created sandbox: id=%s templateID=%s alias=%s", cr.SandboxID, cr.TemplateID, cr.Alias)
